...
from .models import SEARCH_DOC, IndexedModel, IndexedModelManager
...
class SearchDocumentMixinTests(TestCase):
    ...
    def test_search_indexes(self, mock_indexes):
        ...
        mock_indexes.assert_called_once_with(IndexedModel)
    ...
    def test_as_search_document_update_full(self, mock_properties):
        ...
        obj = IndexedModel(simple_field_1=1, simple_field_2="foo")
    ...
    def test_index_search_document(self, mock_client):
        ...
        mock_client.return_value.index.assert_called_once_with(
            body=doc, doc_type="indexedmodel", id=1, index="_all"
        )
    ...
    def test_update_search_document(self, mock_client):
        ...
        obj = IndexedModel(pk=1, simple_field_1=1)
        ...
        mock_client.return_value.update.assert_called_once_with(
            body={"doc": doc}, doc_type="indexedmodel", id=1, index="_all"
        )
    ...
    def test_update_search_document_empty(self, mock_client):
        ...
        with mock.patch.object(
            IndexedModel, "as_search_document_update"
        ) as mock_update:
    ...
    def test_delete_search_document(self, mock_client):
        ...
        mock_client.return_value.delete.assert_called_once_with(
            doc_type="indexedmodel", id=1, index="_all"
        )
...
class SearchDocumentManagerMixinTests(TestCase):
    ...
    @mock.patch.object(IndexedModelManager, "get_search_queryset")
    def test_in_search_queryset(self, mock_qs):
        """Test the in_search_queryset method."""
        obj = IndexedModel(id=1)
        IndexedModel.objects.in_search_queryset(obj.id)
    ...
    def test__raw_sql(self):
        ...
        self.assertEqual(
            IndexedModel.objects._raw_sql(((1, 2), (3, 4))),
            'SELECT CASE tests_indexedmodel."id" WHEN 1 THEN 2 WHEN 3 THEN 4 ELSE 0 END',
        )
    ...
    def test_from_search_query(self, mock_qs):
        ...
        qs = IndexedModel.objects.from_search_query(sq)
        self.assertEqual(
            str(qs.query),
            'SELECT "tests_indexedmodel"."id", "tests_indexedmodel"."simple_field_1", '  # noqa
            '"tests_indexedmodel"."simple_field_2", "tests_indexedmodel"."complex_field", '  # noqa
            '(SELECT CASE tests_indexedmodel."id" WHEN 1 THEN 1 WHEN 2 THEN 2 ELSE 0 END) AS "search_score", '  # noqa
            '(SELECT CASE tests_indexedmodel."id" WHEN 1 THEN 0 WHEN 2 THEN 1 ELSE 0 END) AS "search_rank" '  # noqa
            'FROM "tests_indexedmodel" WHERE "tests_indexedmodel"."id" IN (1, 2) ORDER BY "search_rank" ASC',  # noqa
        )
        ...
        self.assertEqual(
            ...
            'SELECT "tests_indexedmodel"."id", "tests_indexedmodel"."simple_field_1", '  # noqa
            '"tests_indexedmodel"."simple_field_2", "tests_indexedmodel"."complex_field", '  # noqa
            '(SELECT CASE tests_indexedmodel."id" WHEN 1 THEN 0 WHEN 2 THEN 2 ELSE 0 END) AS "search_score", '  # noqa
            '(SELECT CASE tests_indexedmodel."id" WHEN 1 THEN 0 WHEN 2 THEN 1 ELSE 0 END) AS "search_rank" '  # noqa
            'FROM "tests_indexedmodel" WHERE "tests_indexedmodel"."id" IN (1, 2) ORDER BY "search_rank" ASC',  # noqa
        )
